...
#{proposal.property.title} foi aceita"
    ...
    it 'render email body' do
      ...
      p1 = "Eu, João da Silva, portador(a) do CPF nº 111.222.333-44, declaro \
para os devidos
fins que alugo o imóvel sito à #{proposal.property.address}, para Maria da \
Silva,
...
pelo valor total de #{number_to_currency proposal.total_amount}."
      ...
      p2 = "Para este período devem ser respeitados as seguintes regras de uso:
...
#{proposal.property.usage_rules}"
      ...
      p3 = I18n.localize(Date.today, format: :long).to_s

      expect(mail.body).to match('Comprovante de Locação')
      expect(mail.body).to include p1
      expect(mail.body).to include p2
      expect(mail.body).to include p3
    end
  end
end
